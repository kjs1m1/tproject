...
public class ReviewBoardFactory {
    ...
    // 게시판 목록출력 + 검색기능
    public ArrayList<ReviewBoard> boardList(HashMap<String, String> searchList){
        String listSQL = "";
        String val = "";
        ...
        for(String key : searchList.keySet()) {
            switch (key) {
                case "title" :
                    listSQL = "select bdno,userid,title,contents,views,regdate,file1 from reviewboard where title like '%'|| ? ||'%' order by regdate desc";
                    break;
                case "contents":
                    listSQL = "select bdno,userid,title,contents,views,regdate,file1 from reviewboard where contents like '%'|| ? ||'%' order by regdate desc";
                    break;
                case "userid" :
                    listSQL = "select bdno,userid,title,contents,views,regdate,file1 from reviewboard where userid like '%'|| ? ||'%' order by regdate desc";
                    break;
                case "selected" :
                    listSQL = "select bdno,userid,title,contents,views,regdate,file1 from reviewboard order by regdate desc";
                    break;
            }
