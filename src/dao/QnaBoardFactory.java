...
public class QnaBoardFactory {
    ...
    public ArrayList<QnaBoard> qnalist(String searchText) {

        String qnalistSQL = "";

        if (searchText.equals("baselist")){
            qnalistSQL = "SELECT BDNO,TITLE,USERID,REGDATE,THUMB,VIEWS FROM QnaBoard order by REGDATE desc";
        } else {
            qnalistSQL = "SELECT BDNO,TITLE,USERID,REGDATE,THUMB,VIEWS FROM QnaBoard where TITLE like '%'|| ? ||'%' order by REGDATE desc ";
        }
