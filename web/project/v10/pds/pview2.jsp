...
<%
    ...
    Pds p = pdsf.getPdsById(id);  // VO에서 가져옴
%>
...
<div class="main">  <!--https://glyphicons.com/-->
        <div style="padding:30px 0"><i class="fa fa-upload">자료실</i> </div>
       ...
       <table class="table">
           ...
           <tr>
               <td colspan="2">
                   ...
                   첨부파일1 :<a href="pdown.jsp?f=<%=p.getFile1()%>"> <%=p.getFile1()%></a>(다운회수 :123, 크기 : 135kb)</span>
    ...
    <div class="row" style="margin: 10px 30px 20px 30px">
        <img src="showimg.jsp?f=<%=p.getFile1()%>" width="50%">
        <div class="col-6">
...
<script>
    ...
    $(function() { /*함수실행 -> 조인btn에서 클릭이 발생되면 어그리html로 이동해라*/
        $('#listbtn').on('click', function(e) {location.href='<%=baseurl%>/pds/plist.jsp';});
    });

    $(function() { /*함수실행 -> 조인btn에서 클릭이 발생되면 어그리html로 이동해라*/
        $('#updatebtn').on('click', function(e) {location.href='<%=baseurl%>/pds/pmodify.jsp';});
    });
